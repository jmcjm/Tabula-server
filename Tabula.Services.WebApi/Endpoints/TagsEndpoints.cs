...
public static class TagsEndpoints
{
        ...
        ClaimsPrincipal user)
    {
        ...
        if (string.IsNullOrWhiteSpace(model.Name))
            return TypedResults.BadRequest(new { error = "Tag name is required." });

        if (model.Name.Length > 50)
            return TypedResults.BadRequest(new { error = "Tag name cannot exceed 50 characters." });
        ...
        ClaimsPrincipal user)
    {
        ...
        if (tagResult.Value.UserId != currentUserId)
            return TypedResults.BadRequest(new { error = "Tag does not belong to the current user." });
